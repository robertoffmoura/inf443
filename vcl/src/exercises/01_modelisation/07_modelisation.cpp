...
#include "07_modelisation.hpp"

#ifdef INF443_07_MODELISATION
...
void scene_exercise::create_penguin(float scale) {
    ...
    const float r_eye = 0.05f;
    const float r_pupil = 0.01f;
    const float r_body0 = 0.32f;
    const float r_body1 = 0.5f;
    const float r_body2 = 0.22f;
    ...
    mesh_drawable beak = mesh_primitive_cone(0.13f,  {0,0,0}, {0,1.2f*r_head,0}, 20, 20);
    beak.uniform_parameter.color = {1.0f, 0.7f, 0.4f};

    mesh_drawable arm_left = mesh_primitive_quad({0.2f,0.3f,0}, {0.4f,0.2f,0}, {0.4f,0,0}, {0.2f,-0.1f,0});
    mesh_drawable arm_lower_left = mesh_primitive_quad({0,0.2f,0}, {0.2f,0.1f,0}, {0.2f,0,0}, {0,0,0});
    mesh_drawable hand_left = mesh_primitive_quad({0,0.1f,0}, {0.03f,0.07f,0}, {0.03f,0.03f,0}, {0,0,0});

    mesh_drawable arm_right = mesh_primitive_quad({-0.2f,0.3f,0}, {-0.4f,0.2f,0}, {-0.4f,0,0}, {-0.2f,-0.1f,0});
    mesh_drawable arm_lower_right = mesh_primitive_quad({-0,0.2f,0}, {-0.2f,0.1f,0}, {-0.2f,0,0}, {-0,0,0});
    mesh_drawable hand_right = mesh_primitive_quad({-0,0.1f,0}, {-0.03f,0.07f,0}, {-0.03f,0.03f,0}, {0,0,0});
    ...
    hierarchy.add_element(neck, "neck", "body", {0,0.4f,0});
    hierarchy.add_element(head, "head", "neck", {0,0,0.3f});
    ...
    hierarchy.add_element(arm_left, "arm_left", "body", {0, -0.1f, 0});
    ...
    hierarchy.add_element(arm_right, "arm_right", "body", {0, -0.1f, 0});
...
void scene_exercise::draw_penguin(std::map<std::string,GLuint>& shaders, scene_structure& scene) {
    ...
    hierarchy.translation("body") =  {0,0,0.1f*(1+std::sin(2*3.14f*pt))};
